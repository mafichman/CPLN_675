...
author: "Michael Fichman, University of Pennsylvania"
date: "3/5/2025"
output:
  html_document:
    toc: true
...
Recall our site suitability exercise in Week 1 - we used our own spatial decision factors, or "weights" to impose a rule-based system for identifying land areas. We are going to learn an alternative approach, where we use a statistical model to algorithmically determine the weights and use these to predict suitability.

If you have never used logistic regression, or "logit" modeling, make sure you read the class text [*Introduction to Applied Predictive Modeling*](https://mafichman.github.io/applied_predictive_modeling/) to see the basic workflow for training and testing models.

The use case is as follows - assume you are a program manager at a land conservation non-profit, and you are seeking to use your resources to acquire land to conserve. Given all the land in Pennsylvania, it's hard know where to start. It is very labor intensive to inquire about properties, so you need to find a way to identify and porioritize suitable areas to investigate for acquisition. By using a predictive model, you can identify likely lands and begin to research their ownership, and determine whether they are available.
...
*This material is based on in-class exercises developed in conjunction with the late Prof. Ken Steif.*
...
To get started, let's install libraries.
...
Let’s load our data. We are going to include two datasets: `protected_fishnet`, which is a fishnet dataset representing preserved land, and `protected`, which is the original protected lands shapefile.

How do you make a fishnet on your own? You can learn about that in the [Intro to R](https://mafichman.github.io/R_FAQ_For_Planners/#Making_a_%E2%80%9CFishnet%E2%80%9D) course text.

```{r load_data, warning = FALSE, message = FALSE, results = "hide"}
protected_fishnet <- st_read("https://raw.githubusercontent.com/mafichman/CPLN_675/main/Week_7_10/data/pa_conservation/fishnet3k_pa_JoinDEM_Slope_distSlope_distUrban_landCover_distRivers.geojson")
...
```{r transform}
protected_fishnet <- protected_fishnet %>%
...
```{r plot_fishnet}
ggplot() + 
  geom_sf(data=protected_fishnet, aes(fill=as.factor(preserve)), color = "transparent") +
...
Let’s build some bar plots that show differences in our independent variables across land that has and has not been preserved. We create a new data frame called `protectedPlotVariables` that is uniquely shaped to make "facetted plots."
...
protected_fishnet %>% as.data.frame() %>%
    select(preserve,elevation,slope,dSteepSlop,dUrban,distRivers) %>% 
...
**Can you calculate how many fishnet grid cells are preserved?**  

**What is it on a percentage basis?**

**Can you re-scale slope to look at it how it varies from preserved/not-preserved using the 'scales' argument in the facet argument?**
...
Lets filter our `protected_fishnet` data set to keep only the variables we want to analyze. Notice that land cover is an integer but we want it to be a factor, so it gets re-coded.
...
In order to assess our model's accuracy, we are going to split our data into training and test sets. The test set will be put aside as we train our model on the `training` set. We then predict for the test set values using our model and compare the accuracy of our predictions to the known values in the test set to see how well our model performs.
...
*Why do you think `protected_fishnet$landCover` is specified below?*

*Why is it important to have separate training and test sets?*

```{r training_set}
set.seed(3456)
trainIndex <- createDataPartition(protected_fishnet$landCover, p = .70,
                                  list = FALSE,
                                  times = 1)

preserveTrain <- protected_fishnet[ trainIndex,]
preserveTest  <- protected_fishnet[-trainIndex,]
```
...
preserveModel <- glm(preserve ~ elevation + slope + dSteepSlop + landCover + dUrban + distRivers, 
                    family="binomial"(link="logit"), data = preserveTrain %>%                                                       as.data.frame())
...
*Changing your threshold might give you a more accurate model and/or optimize against certain types of error. Watch what happens to the Accuracy and the confusion matrix when you change the threshold in the ifelse statement below.*
...
One of the key techniques mentioned in the *Introduction to Applied Predictive Modeling* text is Cross-Validation.

Why would we want to do k-fold or LOGO Cross-Validation?

Can you describe how that process would work?
...
allPredictions <- 
  predict(preserveModel, protected_fishnet, type="response")
  
protected_fishnet <- 
  cbind(protected_fishnet,allPredictions) %>%
...
```{r predicted_map1, warning = FALSE, message = FALSE}
 ggplot() + 
    geom_sf(data=protected_fishnet, aes(fill=factor(ntile(allPredictions,5))), 
            colour=NA) +
    scale_fill_manual(values = c("#edf8fb","#b3cde3","#8c96c6","#8856a7","#810f7c"),
                      labels=as.character(quantile(protected_fishnet$allPredictions,
                                                 c(0.1,.2,.4,.6,.8),
                                                 na.rm=T)),
                      name="Predicted\nProbabilities(%)\n(Quintile\nBreaks)") +
  theme_void() +
...
```{r predicted_map2, warning = FALSE, message = FALSE}
 ggplot() + 
  geom_sf(data=protected_fishnet, aes(fill=factor(ntile(allPredictions,5))), colour=NA) +
  scale_fill_manual(values = c("#edf8fb","#b3cde3","#8c96c6","#8856a7","#810f7c"),
                    labels=as.character(quantile(protected_fishnet$allPredictions,
                                                 c(0.1,.2,.4,.6,.8),
                                                 na.rm=T)),
                    name="Predicted\nProbabilities(%)\n(Quintile\nBreaks)") +
  geom_sf(data=protected_fishnet  %>% 
               filter(preserve == 1), 
...
```{r}
ggplot() +
  geom_sf(data=st_sf(st_union(protected_fishnet))) +
...
Next, in order to measure ‘nearest neighbor distance’, we have to convert both the unicorn farms and the protected_fishnet to matrices of xy centroid coordinates like so:
...
Now let’s run the function, append unicornDistance to our fishnet centroids data frame and map. Notice, I bind the result to the protected_fishnet data frame with bind_cols.

See what’s going on with the nearest neighbor distance?

```{r}
unicornDistance <- nn_function(protectedXY, unicornXY, 1)

output <- bind_cols(protected_fishnet, unicornDistance) 
...
```{r}
unicornDistance <- nn_function(protectedXY, unicornXY, 3)
...
In order to make other spatial variables for models you are building in R, you might find yourself wanting to summarize vector or raster information inside a fishnet. The equivalent tool to do this in ArcGIS is "Zonal Statistics."
...
We can join our `protected_fishnet` centroids to these polygons. (Why are we joining the centroids to the polygons? Think about the scale biases associated with different types of joins.)
...
protected_and_forests_centroid <-
  st_join(protected_fishnet %>%
            st_centroid(),
          forest_districts)
...
protected_and_forests_fishnet <-
  left_join(protected_fishnet,
            protected_and_forests_centroid %>%
              as.data.frame() %>%
...
parks_and_fishnet <- st_join(local_parks, protected_and_forests_fishnet)
```
...
protected_and_forests_and_parks_fishnet <-
  parks_and_fishnet %>%
  as.data.frame() %>% # Need to make the sf a dataframe to throw away the geometry
  group_by(Id) %>%
  summarize(n_parks = n()) %>%
  right_join(., protected_and_forests_fishnet) %>%
  st_as_sf()
...
```{r last_map}
ggplot()+
  geom_sf(data = protected_and_forests_and_parks_fishnet,

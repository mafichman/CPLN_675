...
**Can you calculate how many fishnet grid cells are preserved?  What is it on a percentage basis?**

# 4. Data wrangling

Lets build another data set of just the variables we want to analyze. Notice that land cover is an integer but we want it to be a factor, so it gets recoded.
...
- `createDataPartition` randomly separates our data into two sets. Why do you think `preserve$landCover` is specified below?
...
Now that we have tuned our model, let’s predict for the entire dataset.
...
```{r park_boundaries, message = FALSE, warning = FALSE}
...
Now let's load in some points representing local parks, and summarize how many points we find in each of our fishnet cells. These data also come from DCNR, and I've turned

---
title: "CPLN 675: Site suitability & predictive modeling"
...
Install these libraries and create a mapTheme()
...
Let’s load our data. We are going to include two datasets: `preserve`, which is our fishnet dataset and `protected`, which is the original protected lands shapefile.
...
Let's project it to PA State Plane, wich has a CRS (coordinate reference system) of 2272.
...
```{r}
ggplot() + 
  geom_sf(data=preserve, aes(fill=as.factor(preserve))) +
...
That figure above takes a long time to plot. Although we are dealing with a fishnet, let’s try to plot as points using `geom_point` instead of `geom_sf`. Note that there are centroid fields, x and Y in the data (note accidentally one is capitalized). These get input into the aes call below. size is outside of aes as it gets a generic parameter, not one from the data frame.

In addition, because we are no longer using fill, we use scale_colour_manual to create a colour gradient.

```{r}
ggplot() + 
  geom_point(data=preserve, aes(x,Y,colour=as.factor(preserve)), size=2) +
  scale_colour_manual(values = c("dodgerblue4", "darkgreen"),
                    labels = c("Not Preserved","Preserved"),
                    name = "") +
  labs(title="Protected lands in Pennsylvania (Points") +
  mapTheme
```

Let’s check out the variables we have in our dataset using the `glimpse` command.
...
Let’s build some bar plots that show differences in our independent variables across land that has and has not been preserved. What is gather doing below?
...
preservePlotVariables <- 
  ...
  gather(key, value, elevation:distRivers)
```

```{r}
ggplot(preservePlotVariables, aes(as.factor(preserve), value, fill=as.factor(preserve))) + 
...
Let’s plot again, but this time we have to scale `slope` so that the differences, if any, are apparent.

```{r}
preservePlotVariables <- 
  preservePlotVariables %>%
  mutate(value = ifelse(key == "slope", value*100, value))
...
*How many fishnet grid cells are preserved? What is it on a percentage basis?*

# Data wrangling

Lets build another dataset of just the variables we want to analyze. Notice that land cover is an integer but we want it to be a factor, so it gets recoded.

```{r}
preserve <- 
  preserve %>%
  select(preserve,elevation,slope,dSteepSlop,landCover,dUrban,distRivers) 

preserve$landCover <- as.factor(preserve$landCover)
```

# Model building

Now we create training and test sets. Why do you think `preserve$landCover` is specified below?
...
Now let’s estimate a logistic regression model. Note how we can use the dplyr pipes right in the data parameter. We have to convert to a data frame because R won’t know how to run a regression on an sf. Also note that we are only training our model on the training set, preserveTrain.
...
Now create a more robust model.

# Model validation

When it’s done, we output classProbs which is the predicted probability of a test set (preserveTest) fishnet cell being conserved conditional on our model. This uses the predict function. type="reponse" ensures that R returns probabilities that range from 0 to 1.
...
Then we build this funky plot, testProbsPlot. What do you think it shows? Can you come up with a title for this plot?
...
Let’s choose 50% and then create a confusion matrix. Below we set the reference to the observed preserved status, data to the predicted outcome, and make sure to state which factor level is the positive (ie. preserved) level. Note that confusionMatrix does not take numeric inputs, only factors.
...
How about an ROC Curve. What does this tell us? See Appendix 1.
...
Testing the power of your model on out of sample data is so critical to the machine learning process. Cross validation iteratively creates many randomly generated test sets or ‘folds’, testing the power of your model on each.
...
cvFit <- train(as.factor(preserve) ~ .,  data = preserve %>% 
                                                as.data.frame() %>%
                                                select(-geometry,-slope,-landCover), 
...
What would you title this map? Note how we use ntile in the aes parameter to create quintiles. The quintile labels are created in the scale_fill_manual function.
...
Let’s map it again with the already other land cover types overlayed.
...
Does this type of model have more, less or the same level of usefullness as a raster-based site suitability? Why?
...
colnames(radarObservations) <- c("Radar_Setting",
                                "Sensitivity__Pct_German_Planes_Detected",
                                "Specificity__Pct_Geese_Flocks_Detected",
                                "Pct_of_Geese_Flocks_Incorrectly_Identified")
...
ggplot(radarObservations, 
       aes(100 - Specificity__Pct_Geese_Flocks_Detected,
           Sensitivity__Pct_German_Planes_Detected)) +
  geom_abline(slope = 1, intercept = 0, size = 1.5, color = 'grey') +
  geom_point(size=2) + 
  geom_line() +
...
Imagine that we believe conservation is really a function of distance to Unicorn Farms and we wish to engineer a feature that was distance from each fishnet grid cell to the nearest unicorm farm. Note that I have given these instructions in ArcMap as part of the interpolation Markdown.

First we have to create our unicorm farms. We do this by taking a random sample of 50 preserve fishnet grid cells, taking their centroids with st_centroid and mapping the mapping them.

...
To get started, let's install libraries and a set of ggplot styles we call `mapTheme`.
...
```{r pa_shp, message= FALSE, warning = FALSE, echo = FALSE}
...
Let's plot the shapefile of our protected lands on top of our counties. We give the shape some color and transparency.
...
That figure above takes a long time to plot - a lot of vector drawing!

Although we are dealing with a fishnet, for speed's sake, let’s try to plot as points using `geom_point` instead of `geom_sf`. Note that there are centroid fields, `x` and `Y` in the data (note accidentally one is capitalized). These get input into the aes call below. size is outside of aes as it gets a generic parameter, not one from the data frame.

In addition, because we are no longer using fill, we use `scale_colour_manual` to create a color gradient.
...
ggplot() + 
  ...
  labs(title="Protected lands in Pennsylvania (Points)",
       subtitle ="This plots much faster!") +
  mapTheme
```
...
![Pivoting between wide and long data](~/Github/CPLN_675/Week_7_10/images/tidyr-spread-gather.gif)
Image: https://www.garrickadenbuie.com/

PLOT LIKE THIS: preservePlotVariables %>% group_by(preserve, key) %>% summarize(mean = mean(value))

Let's examine some of the variables and how they vary across our preserved/not preserved variable.

```{r}
ggplot(preservePlotVariables, aes(as.factor(preserve), value, fill=as.factor(preserve))) + 
     geom_bar(aes(mean), stat="identity") + 
     facet_wrap(~key) +
     scale_fill_manual(values = c("dark blue", "dark green"),
                      labels = c("Not Preserved","Preserved"),
                      name = "") +

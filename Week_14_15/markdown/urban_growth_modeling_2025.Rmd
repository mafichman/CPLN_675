---
title: "CPLN 6750 - Urban Growth Modeling, 2023"
...
Take the functions and stash them in a sepearate .R file.

Explain what they are and what the arguments are, but don't show them, that simplifies things.

Make the workflow correspond to a more present-day time period that is the same as the assignment - 2009-2019 perhaps.

Set up an introduction that describes the workflow in bullet points.

Remove spatial CV - too advanced for 675

Change to workflow so that it's bringing in lc data from both years, identifying change etc., This is the cleaning routine students will be doing.

_This markdown is an updated version of a module written by Ken Steif in 2019. It was an unpublished chapter from his book Public Policy Analytics, and it is therefore very detailed. It has been updated to keep it aligned with current packages and the data sources have been updated for the course github. For the most part, the original text is still used here, out of respect for the original material and the author. However, alterations, annotations and supplemental code are throughout._

_This workflow will form the backbone of your fifth and final assignment for this class. You will build a growth model based on a binary logistic regression that predicts the probability of land cover change at a raster cell level over a specified interval as a function of a) Existing land cover, b) The location of infrastructure and c) Demographic and economic spatial variables._

_While we have used logistic regression to model spatial phenomena like flooding before, this workflow is different in that we wrangle raster data in R rather than doing it in ArcGIS. You do not need to understand the code in full detail, but rather, you should understand what it does operationally, and think about how you can plug in new data from a different time window, in a different geographic context._
...
_Thanks very much to Jenna Epstein for helping to update and edit this material in 2022._

_Michael Fichman, April, 2023_
...
The next section wrangles a host of datasets including, Land Cover and Land Cover Change from the USGS, Census demographics, and transportation. Spatial lag features are also engineered from the land cover change data hypothesizing that the time/space scale of development between 2000 and 2010 can help predict new development in 2020.

Exploratory analysis is undertaken to investigate the relationship between development and the aforementioned features. Findings then motivate the estimation of a geospatial predictive model trained on new development between 2001 and 2011. Those dynamics are then harnessed to predict Development_Demand for 2020 - predictions interpreted as the probability of new development here. The goal is not to create the most accurate model, but to demonstrate the role for geospatial machine learning in the land use modeling process.

Once predictions are validated for their accuracy and spatial generalizability, we evaluate their confluence with environmentally sensitive land in counties throughout the region, with particular emphasis on landscape fragmentation. The final section uses predictions to ‘allocate’ new development across to places where growth can have an economic impact without impeding sustainability goals.

# 1.2. The Workflow

The goal of this analysis is to predict the likelihood of land cover change in the Houston MSA from *undeveloped* to *developed* in year *t* based on demographic, infrastructural, and land cover information from ten years previous (*t-10*). Once calibrated, this model can then be used to take like information from year *t* and project land cover changes in year *t+10*.

This analysis uses the following workflow:

1. Import land cover rasters into R for year *t* and *t-10*

2. Reclassify these rasters into *developed* and *undeveloped*

3. Use map algebra to identify areas that are developed in *t* that were undeveloped in *t-10* - reclassify these as `1` - our outcome of interest.

4. We create a "fishnet" grid for our study area (grid cell polygons), and summarize our raster information into these fishnet cells. Each cell is now a row, with columns

5. We engineer independent variables
...
_MF Note: This chunk differs from the original markdown in a few ways. First, the library `igraph` is now needed. Second, the `mapTheme` and `plotTheme` calls are altered to reflect the default styles we have been using this semester._
...
We also include several helper functions. `quintilesBreaks` takes a dataframe and a column and outputs the quintile breaks, helping shorten the below `ggplot` calls.
...
ggplot() +
  geom_sf(data=houstonMSA) +
  geom_raster(data=rast(development_change) %>% 
                na.omit, 
              aes(x,y,fill=as.factor(value))) +
  scale_fill_viridis(discrete=TRUE, name ="Land Cover\nChange") + 
  labs(title="Development land use change") +

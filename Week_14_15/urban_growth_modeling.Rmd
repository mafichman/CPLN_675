...
author: "Ken Steif, Michael Fichman, Jenna Epstein"
output:
  html_document:
    toc: true
    toc_float: true
...
_This markdown is an updated version of a module written by Ken Steif in 2019. It has been updated to keep it aligned with current packages and the data sources have been updated for the course github. The original text is still used here, but alterations, annotations and supplemental code by Michael Fichman and Jenna Epstein are throughout._
...
_While we have used logistic regression to model spatial phenomena like flooding before, this workflow is different in that we wrangle raster data in R rather than doing it in ArcGIS. You will not be asked to understand the code in full detail, but rather, you should understand what it does operationally, and think about how you can plug in new data from a different time window, in a different geographic context._
...
_MF Note: This chunk differs from the original markdown in a few ways. First, the library `igraph` is now neeed. Second, the `mapTheme` and `plotTheme` calls are altered to reflect the default styles we have been using this semester._
...
library(tigris)
library(FNN)
#library(QuantPsyc) # JE Note: in R 4.1, QuantPsyc package not available. Do we need it?
library(caret)
library(yardstick)
library(pscl)
library(plotROC) 
...
lc_change = raster("data/UGB_Chapter_data/resam4k.tif")
...
```{r}
lc_2001 <- raster("data/UGB_Chapter_data/lc_2001_4k.tif")
...
Both years census data are then plotted.
...
```{r}
lc_2011 <- raster("data/UGB_Chapter_data/lc_2011_4k.tif")

...
Annotations and supplemental code by Jenna Epstein and Michael Fichman.
...
_JE Note: updated land cover change index download link._
...
_JE Note: Will upload a geojson of the houston_MSA shapefile and replace first part of code below to read in the new file_
```{r}
houstonMSA <- 
...
_JE Note: Don't use get_decennial - replace with get_acs. Will create a code vignette for this later. Will need to use an API key, though._

_MF Note: Look at the materials from week 5 (Intro to SF and Tidyverse) and the Intro to R for Planners text._
...
_JE Note: Getting an error that there is a problem with Group.1 in the second block of code in the chunk below. Not sure where Group.1 is created earlier, but it doesn't look like it's in these data frames now. Will try to troubleshoot - I think it's that there isn't an index column present after interpolating, so there isn't a matching field to join by._

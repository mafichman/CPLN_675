...
This is an `R Markdown` document - it's an annotated script of data routines in R.

# Learning objectives

- See how some basic commands function in R that are used to load, examine, manipulate and visualize data.

- Observe how R can be used to wrangle data using commans like `mutate` and `select`.

- See how the `pipe` operator works to chain operations together

- Observe the use of R as a GIS
...
a <- read.csv("~/Github/CPLN_675/Week_2/data/2001.txt")
b <- read.csv("~/Github/CPLN_675/Week_2/data/2011.txt")
```
...
townsShapefile <- read_sf("~/Github/CPLN_675/Week_2/data/ChesterCounty_MuniBoundaries201107.shp")
```

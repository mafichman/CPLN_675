...
Weather, air pollution and elevation are typically represented as continuous spatial processes, but typically derived from discrete sample points like weather stations, air quality sensors and GPS units. This module teaches students how convert discrete samples to continuous spatial ‘surfaces’ and along the way, get an introduction to geospatial prediction.
...
The first exercise will have you perform both IDW and kriging on this data in ArcGIS. Then we will come back to R to take a regression approach.
...
This map is strange. It’s representing grid cells for which there is no air quality sensor as having ozone of 0.000. That’s not quite correct, however. What is the next map doing different?
```{r}
ggplot() +

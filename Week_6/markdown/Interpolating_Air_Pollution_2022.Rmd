...
2. We are going to measure nearest neighbor distance from our fishnet cells to the training set. Using the ‘Near Table’ tool, set the `Mid_Atlantic_EPA_Dataset` as the Input Features (make sure you have unselected any features you might have selected in the last step!) and use `training` as the Near Features. 
...
- We’re not interested in the distances per se - we want to get the average high ozone observation for each of the three nearest grid cells. We join the original `training` shapefile to the `nearTraining` using `NEAR_FID` from the latter to join to `OBJECTID` from the former. Call this new table `nearTraining2`. Now we know the `ozoneHigh` of each training grid cell where there’s an air quality sensor.
...
9. Export this shapefile as `Mid_Atlantic_EPA_Dataset_withLag` Move this shapefile into R. If you can't complete this routine, you can catch up by using a similarly named data set found in the course github (the link to this data set, as a geojson, is below.)


## 3.2. IDW in ArcGIS

**Note - We are going to perform IDW and Kriging and visually compare these between platforms, ArcGIS and R. If you want, you can also follow the instructions denoted as OPTIONAL below, where you can join IDW and Kriging results to our Mid_Atlantic_EPA_Dataset_withLag and bring them into R for comparison.**
...
12. Join this table to `Mid_Atlantic_EPA_Dataset_withLag` by `FID`. If you like, rename the "MEAN" field as `idw_pred`.
...
16. Join this table to `Mid_Atlantic_EPA_Dataset_withLag` by `FID`. If you like, rename the "MEAN" field as `krige_pred`.

17. Output the data set.
...
# Appendix - Creating The Fishnet in R

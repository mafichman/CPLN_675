...
11. Use `Zonal Statistics as Table` to summarize `idw_1` by `Mid_Atlantic_EPA_Dataset` using the Zone Field `FID`. Choose statistics type "MEAN" and call the output `idw_zonal`. Open an edit session - go into the table and rename the field `MEAN` as `idw_pred`.

12. Join this table to `Mid_Atlantic_EPA_Dataset_withLag` by `FID`.
...
15. Use `Zonal Statistics as Table` to summarize `krige_1` by `Mid_Atlantic_EPA_Dataset` using the Zone Field `FID`. Choose statistics type "MEAN" and call the output `krige_zonal`. Open an edit session - go into the table and rename the field `MEAN` as `krige_pred`.

16. Join this table to `Mid_Atlantic_EPA_Dataset_withLag` by `FID`.

17. Output the data set as `Mid_Atlantic_EPA_Dataset_Lag_IDW_Kr`.
...
Take your data set `Mid_Atlantic_EPA_Dataset_Lag_IDW_Kr` - the one with the IDW and Kriging predictions, and load it into R by modifying the following code:

```{r bonus_upload, eval=FALSE, message=FALSE, warning=FALSE}
lag_krige_idw <- st_read("myfilepath/myfile.shp") %>%
  st_transform(crs = 4326)
```

Can you join `lag_krige_idw` to `epa2` and examine differences between `reg.lag.pred` (our regression-generated predictions) and the IDW and kriging estimates? Do this either visually or through summary.

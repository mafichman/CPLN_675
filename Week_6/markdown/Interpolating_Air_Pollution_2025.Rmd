---
title: "CPLN 6750: Estimating Ozone Pollution with Spatial Interpolation and Regression"
...
*The course text ["Introduction to Applied Predictive Modeling"](https://mafichman.github.io/applied_predictive_modeling/) contains sample workflows for linear and binomial predictive models and describes key terms and engineering strategies.*

# 1. Introduction

Weather, air pollution and elevation are typically represented as continuous spatial processes. However, they are often derived from discrete sample points like weather stations, air quality sensors and GPS units and *interpolated* into surfaces. This module teaches students how convert discrete samples of data to continuous spatial estimated ‘surfaces’. It also introduces regression-based geospatial predictive modeling.
...
3. To specify how many nearest neighbors to measure distance to, uncheck ‘Find only closest feature’ and set ‘Maximum number of closest features to 3’. Call this new table `nearTraining`. This table stores the relationships between each cell in the Mid-Atlantic dataset and the three nearest ozone readings.
...
4. We want to get the average high ozone observation for each of the three nearest observations, so we have to join the training data (which contain the ozone readings) to the near table (which only has the spatial relationships between features). So - we need to impart the `ozoneHigh` measurement to the `NEAR_FID` nearest neighbor cells in `nearTraining`.

- Using the "joins and relates" menu in your catalog pane, initiate a join from the `training` shapefile to the `nearTraining`.  For the join fields, use `OBJECTID` from `training` (this might also be called `FID`, depending on your version of ArcGIS). Use `NEAR_FID` as the join field from `nearTraining`. 

The join adds new columns to the `training` shapefile with the `ozoneHigh` associated with our nearest neighbor cells.
...
6. We’re going to calculate the average `ozoneHigh` of each cell’s 3 nearest training neighbors. 

- Add `nearTraining2` to your viewer; open the attribute table; right click on `IN_FID` and click `summarize`. Drop down the `ozoneHigh` field and make the statistic type `MEAN`. Name this table `nearTraining3` and run the tool.
...
Let’s start by loading the requisite libraries, reading the epa shapefile and plotting it using the native sf plotting function. Note that if you are new to the sf package you should go run-through this tutorial.
...
```{r}
ggplot() +
  geom_sf(data=epa , aes(fill=ozoneHigh), color = "transparent") + 
  scale_fill_viridis() +
...
For our first step, we are going to clean up our data a bit.


- We create a new sf object called `epa2`. We pull out the variables we want for this analysis from `epa` using `select`. Examine what we keep - some variables associated with the landscape and land use, and our Ozone lag from ArcGIS - `Ave_ozoneH`.
...
```{r}
ggplot() +
    geom_sf(data = epa2, aes(fill=distWater)) + 
    scale_fill_viridis(name="Dist Water") +
    theme_void()
```
...
- We want to create a data frame called `scatterplot_data` that's only our independent and dependent variables.

- We convert `training` to a data frame (e.g. a flat file) using `as.data.frame`, and so we drop the columns `training` and `geometry` using a `select` command.
...
scatterplot_data <- training %>% 
  as.data.frame() %>%
  dplyr::select(-training, -geometry) %>%
  gather(Variable, Value, -ozoneHigh)

    ggplot(data = scatterplot_data) +
      geom_point(aes(x = Value, y = ozoneHigh)) +
      geom_smooth(aes(x = Value, y = ozoneHigh), method = "lm", se=FALSE) +
...
```{r}
reg <- lm(data=training, ozoneHigh ~ HwyDensity + distWater + sumDevelop + sumForest + distI95 + Population + distCities)
...
ggplot(data= training) +
 geom_point(aes(x = Ave_ozoneH, y = ozoneHigh)) +
 geom_smooth(aes(x = Ave_ozoneH, y = ozoneHigh), method="lm",se=F) +
...
```{r}
reg.lag <- lm(data=training, ozoneHigh ~ HwyDensity + distWater + sumDevelop + sumForest + distI95 + Population + distCities + Ave_ozoneH)
...
training.summary <-
  ...
  as.data.frame() %>%
    ...
    dplyr::select(ozoneHigh, reg1.Pred, reg.lag.Pred) %>%
    gather(Variable, Value, -ozoneHigh) 

glimpse(training.summary)
```
...
ggplot(data = training.summary) + 
  geom_point(aes(y = Value, x =ozoneHigh)) + 
  geom_smooth(aes(y = Value, x = ozoneHigh), method = "lm", se = FALSE) +
...
`absError` is the absolute value of the difference between observed ozoneHigh and the predicted. 

Notice that `group_by` allows us to calculate the average error by each regression type. 

*Why do you think we are interested in absolute error rather than raw error?*

***Which model is the strongest?***

*Can you take `training.summary` and create MAPE in the mutate statement?*
...
```{r}
epa2 <- epa2 %>%
  mutate(reg.lag.pred = predict(reg.lag, .))
```
...
Notice how we can filter our data set `epa2` inside the ggplot call - this is a nice shortcut to keep us from having to make a whole new data frame.

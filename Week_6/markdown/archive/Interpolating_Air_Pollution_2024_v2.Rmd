---
title: "CPLN 6750: Interpolating Air Pollution Dynamics with Interpolation and Regression"
...
The second part of the module involves running an **OLS regression** model in R to estimate ozone levels.

Our regression analysis is designed to predict local ozone as a function of independent predictors about our study area (like highway density and population density) and spatial variables (like nearby ozone measurements).  We can compare the accuracy of this method to our spatial interpolations.

## 1.2. Learning Objectives

- Understand the idea of spatial interpolation. Compare and contrast methodological approaches.

- Use the "fishnet" as a vector alternative to raster GIS operations.

- Run a predictive model using spatial variables: run some linear models and see how they work mechanically when used for prediction.
...
Let’s start by loading some libraries, and reading the epa shapefile and plotting it using the native sf plotting function. Note that if you are new to the sf package you should go run-through this tutorial.
...
Can you create this map you see below on your own by creating two `geom_sf` objects? 

One should be just be a black shape, and a second on top should only contain our training cells.
...
- We use the `st_centroid` and `st_coordinates` commands from `sf` in a series of `mutate` statements. First we extract the centroid from the geometry to a column we call `centroid`, and then we use `st_coordinates` to access the ordered list of centroid coordinates - using matrix notation (eg [,1]) to call the first item in the list as the `X` and second as the `Y`. 
...
Let’s estimate a ‘kitchen sink’ regression with all of our variables **except** the spatial variables.
...
```{r}
reg <- lm(ozoneHigh ~ HwyDensity + distWater + sumDevelop + sumForest + distI95 + Population + distCities, data=training)
...
```{r}
reg2 <- lm(ozoneHigh ~ HwyDensity + distWater + sumDevelop + 
             sumForest + distI95 + Population + distCities +
             X + Y, 
           data=training)
...
```{r}
reg.lag <- lm(ozoneHigh ~ HwyDensity + distWater + sumDevelop + 
             sumForest + distI95 + Population + distCities +
             X + Y + Ave_ozoneH, 
           data=training)
summary(reg.lag)
...
training.summary <-
  training %>% 
    ...
    dplyr::select(c(ozoneHigh, reg1.Pred, reg2.Pred, reg.lag.Pred)) %>%
...
ggplot(data = training.summary, aes(y = Value, x =ozoneHigh)) + 
